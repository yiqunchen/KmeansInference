...
Note that our `kmeans_estimation` function is an `R` implementation of the `kmeans` function in base `R` that stores *all the intermediate clustering assignments* (see Section 3 of our manuscript for details). We see below that the estimated clusters, *as well as their ordering*, agree between these two functions. 

N.B.: the `kmeans` function in base `R` was implemented in Fortran and C, while our implementation is entirely in `R`. As a result, there could be corner cases where these two functions disagree. 
```{r fig.align="center",  fig.height = 4, fig.width = 5}
set.seed(2021)
