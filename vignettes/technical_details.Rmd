...
$$\mathbf{X} \sim \mathcal{MN}_{n\times q}({\mu}, \textbf{I}_n, \sigma^2 \textbf{I}_q),$$
where $\mu \in \mathbb{R}^{n\times q}$ has unknown rows $\mu_i$, and $\sigma^2 > 0$ is unknown. This is equivalent of positing that $X_i\sim_{ind.} \mathcal{N}(\mu_i,\sigma^2 \textbf{I}_q)$. In addition, we use $x$ to denote a realization of the data generating model $\mathbf{X}$.
...
Moreover, if we let $\mathcal{S}_T$ denote $\left\{ \phi\geq0: \bigcap_{t=0}^{T}\bigcap_{i=1}^{n} \left\{c_i^{(t)}\left(\mathbf{x}'(\phi)\right) = c_i^{(t)}\left(\mathbf{x}\right)\right\}   \right\}$, then $p_{k-means}$ can be rewritten as $\mathbb{P}\left( \phi \geq \Vert\mathbf{x}^\top \nu \Vert_2 \;\middle\vert\; \phi\in \mathcal{S}_T   \right)$. Therefore, it suffices to characterize the set $\mathcal{S}_{T}$, because $\phi \;\middle\vert\; \phi\in \mathcal{S}_T $ will follow a truncated $\chi$ distribution.
...
1. Allow for inference with unknown $\sigma$, i.e., where $\mathbf{X} \sim \mathcal{MN}_{n\times q}({\mu}, \textbf{I}_n, \sigma^2 \textbf{I}_q)$ with *unknown* $\sigma$.
2. Allow for inference with a known, positive-definite covariance matrix $\Sigma$, i.e., where $\mathbf{X} \sim \mathcal{MN}_{n\times q}({\mu}, \textbf{I}_n, \Sigma)$.
