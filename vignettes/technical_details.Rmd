...
$$X \sim \mathcal{MN}_{n\times q}({\mu}, \textbf{I}_n, \sigma^2 \textbf{I}_q),$$
where $\mu \in \mathbb{R}^{n\times q}$ has unknown rows $\mu_i$, and $\sigma^2 > 0$ is unknown. This is equivalent of positing that $X_i\sim_{ind.} \mathcal{N}(\mu_i,\sigma^2 \textbf{I}_q)$. In addition, we use $x$ to denote a realization of the data generating model $X$.
...
$$   H_0: \sum_{i\in {\hat{\mathcal{C}}}_1}\mathbf{\mu}_i/|\hat{\mathcal{C}}_1| = \sum_{i\in \hat{\mathcal{C}}_2}\mathbf{\mu}_i/|\hat{\mathcal{C}}_2|   \mbox{ versus }  H_1: \sum_{i\in \hat{\mathcal{C}}_1}\mathbf{\mu}_i/|\hat{\mathcal{C}}_1| \neq \sum_{i\in \hat{\mathcal{C}}_2}\mathbf{\mu}_i/|\hat{\mathcal{C}}_2|,
$$
where $\hat{\mathcal{C}}_1,\hat{\mathcal{C}}_2 \in \mathcal{C}({x})$ are estimated clusters with cardinalities $|\hat{\mathcal{C}}_1|$ and $|\hat{\mathcal{C}}_2|$. This is equivalent to testing
...
$$
where $\phi\sim (\sigma\Vert\nu\Vert_2)\cdot\chi_q$ is a scaled $\chi$ distribution with degrees of freedom $q$, and  ${x}'(\phi) =  x + \left( \frac{\phi-\Vert {x}^{\top}\nu  \Vert_2 }{\Vert \nu  \Vert_2^2 }\right) \cdot \nu \left({\text{dir}}({x}^{\top}\nu)\right)^{\top}$ can be thought of as a perturbation of the observed data $x$, along the direction of ${x}^{\top}\nu$.

Moreover, if we let $\mathcal{S}_T$ denote $\left\{ \phi\geq0: \bigcap_{t=0}^{T}\bigcap_{i=1}^{n} \left\{c_i^{(t)}\left({x}'(\phi)\right) = c_i^{(t)}\left({x}\right)\right\}   \right\}$, then $p_{k-means}$ can be rewritten as $\mathbb{P}\left( \phi \geq \Vert{x}^\top \nu \Vert_2 \;\middle\vert\; \phi\in \mathcal{S}_T   \right)$. Therefore, it suffices to characterize the set $\mathcal{S}_{T}$, because $\phi \;\middle\vert\; \phi\in \mathcal{S}_T $ will follow a truncated $\chi$ distribution.
...
1. Allow for inference with unknown $\sigma$, i.e., where $X \sim \mathcal{MN}_{n\times q}({\mu}, \textbf{I}_n, \sigma^2 \textbf{I}_q)$ with *unknown* $\sigma$.
2. Allow for inference with a known, positive-definite covariance matrix $\Sigma$, i.e., where $X \sim \mathcal{MN}_{n\times q}({\mu}, \textbf{I}_n, \Sigma)$.
