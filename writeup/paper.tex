...
of clustering. The phenomenon is generic: even on pure noise, $k$-means returns
clusters whose mean differences a naive test declares overwhelmingly significant
(Figure~\ref{fig:intro}).

\begin{figure}[t]\centering
  \includegraphics[width=\textwidth]{intro_motivation.png}
  \caption{The double-dipping problem. \textbf{(a)} One null dataset
  ($\bmu=\bm 0$); $k$-means with $K=3$ nonetheless returns three clusters
  (centroids as triangles). \textbf{(b)} QQ-plot of the naive $p$-values for the
  cluster-mean differences over many such null datasets: they collapse far below
  the $45^\circ$ line --- grossly anti-conservative. \textbf{(c)} The selective
  path and union $p$-values are uniform, controlling the selective Type I error.
  Source: \texttt{sims/plot\_intro\_motivation.R}.}
  \label{fig:intro}
\end{figure}
...
\begin{figure}[t]\centering
  \includegraphics[width=0.8\textwidth]{decomposition.png}
  ...
  ($\to\chi$); the studentized test (Section~\ref{sec:rfiber}) fixes the total $T$
  and varies the ratio $R$ ($\to F$).
  Source: \texttt{sims/plot\_decomposition.R}.}
...
\emph{algorithmic trace} (the Lloyd path). Figure~\ref{fig:condset} illustrates
why this buys power: the union set reaches values of $\phi$ \emph{below} the
observed statistic --- smaller separations at which the same two clusters are
still recovered --- which the path test discards.

\begin{figure}[t]\centering
  \includegraphics[width=\textwidth]{conditioning_set.png}
  \caption{Condition on less $\Rightarrow$ more power, on one dataset
  ($p_{\mathrm{path}}=0.83$, $p_{\mathrm{union}}=5\times10^{-4}$).
  \textbf{Top:} the data perturbed along the contrast, $\x'(\phi)$.
  \textbf{(a)} far below the conditioning set the two tested clusters merge
  (recoloured); \textbf{(b)} at a smaller separation inside the union set --- but
  outside the path interval --- they are still recovered; \textbf{(c)} the observed
  data. \textbf{Bottom:} the set of $\phi$ preserving the two clusters. The path
  test conditions on the single interval at the observed statistic (dashed); the
  union test on the whole set, which extends below it. Truncated-$\chi$ null
  underneath. Source: \texttt{sims/plot\_conditioning\_set.R}.}
  \label{fig:condset}
\end{figure}
...
Figure~\ref{fig:exactis} makes the first point concrete: running
\citet{yun2023unknownvar}'s own importance sampler on the same $k$-means
clustering, its estimate of the union $p$-value converges to our exact value but
carries Monte Carlo error at every budget --- even at their recommended $8000$
draws --- whereas our value is exact.

\begin{figure}[t]\centering
  \includegraphics[width=0.72\textwidth]{exact_vs_is.png}
  \caption{Exact union $p$-value vs.\ importance sampling, on one $K=3$ dataset.
  The black line is our exact $p=0.0140$. Boxplots are $20$ runs of
  \citet{yun2023unknownvar}'s importance sampler (their proposal, rotation, and
  weights, applied to the $k$-means clustering) at each draw budget: the estimate
  is unbiased for our value but never loses its sampling variance, which our exact
  solver eliminates. Source: \texttt{sims/plot\_exact\_vs\_is.R}.}
  \label{fig:exactis}
\end{figure}
...
\begin{figure}[t]\centering
  ...
  naive $p$-value (orange) collapses below it.
  Source: \texttt{sims/plot\_cw\_typeI.R}.}
  \label{fig:typeIqq}
\end{figure}
...
which handles any $K$. The gain is also robust to cluster geometry: across
horizontal ($K=2$), equilateral-triangle, and collinear ($K=3$) configurations the
union dominates the path at every separation (Figure~\ref{fig:geom}).
Following \citet{chen2023kmeans} we separate
...
\begin{figure}[t]\centering
  ...
  (solid); the gap is largest in the moderate-signal regime.
  Source: \texttt{sims/plot\_cw\_power.R}.}
  \label{fig:cwpower}
\end{figure}

\begin{figure}[t]\centering
  \includegraphics[width=\textwidth]{power_geometries.png}
  \caption{Union vs.\ path power (known $\sigma$, $95\%$ CI) across cluster
  geometries: $K=2$ horizontal, $K=3$ equilateral triangle, $K=3$ collinear
  (Yun--Barber Fig.~4 settings). The union (dashed) is more powerful than the path
  (solid) in every configuration. Source: \texttt{sims/plot\_power\_geometries.R}.}
  \label{fig:geom}
\end{figure}
...
\begin{figure}[t]\centering
  ...
  \caption{Palmer penguins, $K=3$ (Table~\ref{tab:penguins}). Left: the four
  standardized features in the first two PCs, true species as point shape and
  $k$-means clusters as colour. Right: the selective $p$-value ($-\log_{10}$) for
  each cluster pair, by method. Source: \texttt{penguins.png}.}
  \label{fig:penguins}
\end{figure}
...
% ===========================================================================
\section{The role of AI assistance}
\label{sec:ai}
% ===========================================================================
In the interest of transparency we document the substantial role that large
language models played in this work. The implementation --- the
\texttt{KmeansInference} package, the exact $R$-fiber solver, the simulation
harness, and every figure --- together with the simulation design and validation
and a first draft of this manuscript, was produced by an agentic coding assistant:
Anthropic's Claude Opus~4.8, operating through Claude Code, under the author's
direction. Specific theoretical claims --- most importantly the validity of the
studentized union on the $R$-fiber --- were independently cross-checked by a
second model, OpenAI's GPT-5.5, whose responses were pasted back into the
development loop.

Several of the conceptual steps were AI-assisted. \textbf{(i)} The
``condition on less'' union construction (Section~\ref{sec:union}) was obtained by
transporting the graph-fused-lasso argument of \citet{chen2023gfl} to $k$-means,
together with the observation that the Lloyd-path exploration is purely Euclidean
and is therefore reused verbatim under a general covariance. \textbf{(ii)} For
unknown variance, an initial proposal --- re-scaling the known-$\sigma$ truncation
set into the studentized $F$ --- was found, through exhaustive simulation, to be
anti-conservative; the assistant then identified the valid construction, forming
the union in the $F$-statistic's own coordinate (the $R$-fiber,
Section~\ref{sec:rfiber}), a claim the second model and our simulations both
confirmed. \textbf{(iii)} The exact $R$-fiber solver rests on recognizing that the
cluster-assignment margins along the fiber are quartic in $\tan(\theta/2)$, which
yields the arc boundaries in closed form and turns the $K>2$ $p$-value --- which
\citet{yun2023unknownvar} estimate by importance sampling --- into an exact,
zero-variance quantity (Figure~\ref{fig:exactis}). \textbf{(iv)} The
``studentized-away'' mechanism (Section~\ref{sec:rfiber}) --- that the union's
power gain is a known-variance phenomenon that nearly vanishes under the
studentized $F$ --- was diagnosed with targeted simulations.

We emphasize that these were AI-\emph{assisted} insights rather than autonomous
proofs: each was validated by exhaustive simulation and a unit-tested
implementation, and the author is responsible for the final mathematics. We make
this disclosure because the division of labour between human and machine is itself
informative for how such tools might be used in methodological statistics.
